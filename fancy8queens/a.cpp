...
bool ok(int q[]){
  ...
  for(int i=7;i>0; i--){
    int j=q[i];
    for(int k = 1; k <= i; k++){
      if(q[i-k] == j) test = false; //row test
      //diagonal tests
      else if (q[i-k] == j-k) test = false;
      else if (q[i-k] == j+k) test = false;
    }
  }
  return test;
...
void print(int q[]){
  ...
  cout << "\nSolution " << ++count << endl;
  for(int i=0;i<8;i++){ //declaring variables and substitutes
    for(int j=0;j<8;j++){
      if(j == q[i]) cout << "1 ";
      else cout << "0 ";
    }
            ...
            for(int i5=0;i5<8;i5++)
              for(int i6=0;i6<8;i6++)
                for(int i7=0;i7<8;i7++){
                  ...
                  q[6]=i6;
                  q[7]=i7;
                  if(ok(q)) print(q); // making sure everything runs well without errors and the steps said to do
  }
  return 0;
